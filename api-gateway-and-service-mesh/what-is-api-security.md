...
**Authorisation** — verify what they may do, on _every_ request and _every_ object. Broken object-level authorisation (BOLA) is the number one item on the OWASP API Top 10: an endpoint that returns `/orders/{id}` without checking the order belongs to the caller is the most common serious API vulnerability in the wild.
